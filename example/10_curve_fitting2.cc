...
std::vector<std::pair<double, double>> GenerateData(double start, double end, double interval, std::function<double(double)> function, double noise = 0.0){
...
int main(int argc, char** argv) {
  ...
  std::vector<std::pair<double, double>> data = GenerateData(-10.0, 10.0, 0.3, function, 5.0);
  ...
  for (int i = 0; i < data.size(); i++) {
    problem.AddResidualBlock(
        new AutoDiffCostFunction<CustomResidual, 1, 1, 1, 1>(   // 1 residual, 3 variable
